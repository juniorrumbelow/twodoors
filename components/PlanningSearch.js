      ...
      <div className="relative mb-10 w-full max-w-3xl mx-auto z-50" ref={dropdownRef}>
        <form onSubmit={handleSearch} className="relative flex items-center justify-center group w-full">
          <input
            ...
            placeholder="Search a UK city, town, or enter postcode..."
            className="w-full px-8 py-5 text-lg font-medium text-gray-900 bg-white border border-gray-200 rounded-full shadow-2xl focus:outline-none focus:border-[#01bf8f] focus:ring-4 focus:ring-[#01bf8f]/20 transition-all placeholder:text-gray-400"
          />
          <button
            type="submit"
            disabled={loading}
            className="absolute right-2 top-2 bottom-2 px-8 py-2 bg-[#01bf8f] text-white font-bold rounded-full hover:bg-[#02a97e] transition-all duration-300 shadow-md hover:shadow-lg disabled:opacity-50 disabled:hover:scale-100 flex items-center gap-2 z-10"
          >
            {loading ? (
              <>
